...
export default function AdminMyUserList() {
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-level', 'my-user-list'],
    queryFn: () => getAdminUsers({ page: 1, limit: 500 }),
  });
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-600">User list available to admin.</p>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[900px] w-full table-fixed divide-y divide-slate-200">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-slate-500">No users found.</td></tr>
                ) : rows.map((u) => (
                  <tr key={u._id}>
