...
export default function AdminAllHierarchy() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-level', 'all-hierarchy', appliedSearch.searchName, appliedSearch.searchMobile, page],
    queryFn: () =>
      getAdminUsers({
        page,
        limit: PAGE_SIZE,
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-600">
        All users in the system. Click a user to open their referral downline (same as My Hierarchy for that root).
      </p>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <div className="min-w-[12rem] flex-1">
            <label htmlFor="all-hierarchy-search-name" className="block text-xs font-medium text-slate-700">
              Name
            </label>
            <input
              id="all-hierarchy-search-name"
              type="text"
              value={nameDraft}
          ...
          <div className="min-w-[12rem] flex-1">
            <label htmlFor="all-hierarchy-search-mobile" className="block text-xs font-medium text-slate-700">
              Mobile
            </label>
            <input
              id="all-hierarchy-search-mobile"
              type="text"
              inputMode="numeric"
              value={mobileDraft}
