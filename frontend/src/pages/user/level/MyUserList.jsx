...
export default function UserMyUserList() {
  const { data, isLoading, error } = useQuery({
    queryKey: ['user-level', 'my-user-list'],
    queryFn: () => getReferralTree(12),
  });
  const rows = flattenTree(data?.data?.tree ?? null, []);
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-600">All users in your hierarchy list.</p>
            ...
            <table className="min-w-[720px] w-full table-fixed divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="w-1/5 px-4 py-3 text-left text-xs font-medium uppercase text-slate-500">Level</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {rows.length === 0 ? (
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-slate-500">No users found.</td></tr>
                ) : rows.map((u) => (
                  <tr key={u.id}>
