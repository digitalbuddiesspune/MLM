...
export default function UserAllHierarchy() {
  const { data, isLoading, error } = useQuery({
    queryKey: ['user-level', 'all-hierarchy'],
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-600">Complete hierarchy in your accessible scope.</p>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[720px] w-full table-fixed divide-y divide-slate-200">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.length === 0 ? (
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-slate-500">No hierarchy data found.</td></tr>
                ) : rows.map((u) => (
                  <tr key={u.id}>
